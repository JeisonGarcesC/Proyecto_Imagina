...
// 2. Parales Blancos KUAC1040000_74.glb (4 columnas elevables)
const paralFiles = [
  { pos: [-0.55, 0, 0.34], rot: [0, 0, 0] },
  { pos: [0.52, 0, 0.34], rot: [0, 0, 0] },
  { pos: [-0.55, 0, -0.26], rot: [0, Math.PI, 0] },
  { pos: [0.52, 0, -0.26], rot: [0, Math.PI, 0] },
];

for (const p of paralFiles) {
  const paral = await loadGlbLocal('public/assets/models/Kuo AV/Puesto Doble/KUAC1040000_74.glb');
  paral.position.set(...p.pos);
  paral.rotation.set(...p.rot);
  paral.traverse((child) => {
...
console.log(`\n=== ENSAMBLE CON PARALES BLANCOS KUAC1040000 ===`);
