...
import {
  ...
  lockerBayCatalog,
  LOCKER_COLUMNS,
} from '../src/mepal/lockers/catalog/lockerCatalog.js';
...
import {
  ...
  getLockerBayWidth,
} from '../src/mepal/lockers/layout/lockerLayout.js';
import {
  createLockerBodyGeometry,
  createLockerAnchorGeometry,
  createLockerLockGeometry,
  disposeLockerGeometry,
} from '../src/mepal/lockers/renderers/LockerNativeRenderer.js';
...
const configFor = (body, material, bayCount) => ({
  calibre: body.calibre,
  lockerType: body.type,
...
test('catálogo: cada código y descripción coincide con la celda original, sin ejecutar macros', () => {
  const book = XLSX.read(
    fs.readFileSync(
      new URL(
        '../public/assets/MapaProductoYFichasTecnicas/Lockers/ayudaVentasLockers.xlsm',
        import.meta.url
      )
    ),
    { type: 'buffer' }
...
for (const body of lockerBodyCatalog)
  test(`coraza ${body.code}: módulos, materiales, kits 1–4 y BOM`, () => {
    for (const material of ['METALICA', 'METALICA_EMBEBIDA', 'FORMICA', 'MELAMINA'])
      for (const count of [1, 2, 3, 4]) {
        const config = configFor(body, material, count),
          result = createLockerInstance({ config });
        ...
        const root = result.object,
          columns = LOCKER_COLUMNS[body.type];
        assert.equal(root.userData.codigoPT, body.code);
        assert.equal(bays(root).length, columns * count);
        assert.equal(
          root.getObjectByName('BODY').children.filter((n) => n.name.startsWith('DIVIDER_')).length,
          columns - 1
        );
        ...
        assert.equal(
          root.userData.bom.some((r) => r.role === 'SHELF'),
          false
        );
        assert.equal(root.userData.shape2D.geometry.widthMm, body.widthMm);
        assert.equal(
          root.getObjectByName('LEFT_SIDE').geometry.parameters.height,
          body.heightMm / 1000
        );
        root.traverse((node) => {
          if (node !== root) assert.equal(node.userData.excludeFromBOM, true);
        });
        disposeLockerGeometry(root);
      }
  });

test('lockers usa catálogo de acabados siguiendo el estándar del proyecto', () => {
  const result = resolveLockerProduct({
    material: 'FORMICA',
    bodyFinishId: 'FORMICA_30',
    bayFinishIds: ['FORMICA_30', 'MELAMINA_25'],
    bayCount: 2,
    bayFinishes: ['#ff0000', '#00ff00'],
  });
  assert.equal(result.supported, true);
  assert.equal(result.config.bodyFinishId, 'FORMICA_30');
  assert.equal(result.config.bayFinishIds.length, 2);
  assert.equal(result.config.bodyFinishCode, 'LOCKER_FORMICA_30');
  assert.equal(result.config.bayFinishCodes[1], 'LOCKER_MELAMINA_25');
  assert.equal(result.config.bayFinishes[0], '#ff0000');
  assert.equal(result.config.bayFinishes[1], '#00ff00');
});
...
test('sin interpolación ni sustituciones para dimensiones, calibres o materiales desconocidos', () => {
  for (const patch of [
    ...
    const result = resolveLockerProduct(patch);
    assert.equal(result.supported, false);
    assert.equal(result.codeResolution.code, null);
    assert.deepEqual(result.bom, []);
  }
  assert.equal(
    resolveLockerProduct({ calibre: 22, material: 'METALICA_EMBEBIDA', securityType: 'NO_APLICA' })
      .supported,
    false
  );
});
...
test('restricciones comerciales de seguridad, manija y troquelado', () => {
  for (const material of ['FORMICA', 'MELAMINA']) {
    const r = resolveLockerProduct({
      material,
      punchingType: 'NO_APLICA',
      securityType: 'PORTACANDADO',
    });
    assert.equal(r.supported, false);
    assert.ok(r.diagnostics.some((d) => d.code === 'LOCKER_INVALID_SECURITY'));
    assert.equal(
      resolveLockerProduct({ material, punchingType: 'TIPO_1', punchingPosition: 'ARRIBA' })
        .supported,
      false
    );
  }
  ...
  assert.equal(
    resolveLockerProduct({ securityType: 'CLAVE_4_DIGITOS', handleType: 'SIN_MANIJA' }).supported,
    true
  );
  for (const securityType of ['ARMSTRONG', 'TIMBERLINE'])
    assert.equal(resolveLockerProduct({ securityType }).supported, false);
  assert.equal(resolveLockerProduct({ anchor: true }).supported, false);
  const pending = resolveLockerProduct({
    calibre: 24,
    material: 'METALICA_EMBEBIDA',
    securityType: 'NO_APLICA',
    bayCount: 1,
  });
  ...
  assert.equal(
    resolveLockerProduct({ punchingType: 'LISO', punchingPosition: 'ARRIBA' }).supported,
    false
  );
});
...
test('materiales independientes por nave y de la coraza, antes y después de reconstruir', () => {
  const { object } = createLockerInstance({
    config: { bayCount: 4, bayFinishes: ['#ff0000', '#00ff00', '#0000ff', '#ffffff'] },
  });
  ...
  materials[0].color.set('#123456');
  assert.equal(materials[1].color.getHexString(), '00ff00');
  materials[1].color.set('#654321');
  assert.equal(materials[0].color.getHexString(), '123456');
  ...
  assert.equal(object.userData.instanceId, instanceId);
  assert.deepEqual(object.position.toArray(), [2, 3, 4]);
  assert.equal(object.rotation.y, 0.4);
  assert.equal(
    bays(object)[0].getObjectByName('DOOR_PANEL').material.color.getHexString(),
    'ff0000'
  );
...
test('crecimiento reconstruye ancho, profundidad y altura sin escalar la coraza', () => {
  ...
  assert.equal(
    rebuildLockerInstance({ object, patch: { lockerType: 'DOBLE', widthMm: 1000, heightMm: 2200 } })
      .success,
    true
  );
  ...
  assert.equal(bays(object).length, 2);
  assert.deepEqual(object.scale.toArray(), [1, 1, 1]);
  const technicalBody = createLockerBodyGeometry(
    { widthMm: 400, heightMm: 1700, depthMm: 300, bodyFinish: '#ffffff' },
    1
  );
  ...
  assert.equal(
    resolveLockerProduct({ widthMm: 400, heightMm: 1700, depthMm: 300 }).supported,
    false
  );
...
test('accesorios, cantidades y componentes incluidos sin duplicaciones', () => {
  ...
  assert.equal(
    object.getObjectByName('VIEWER').getObjectByName('ACRYLIC_VIEWER').material.transparent,
    true
  );
...
for (const type of ['TIPO_1', 'TIPO_2', 'TIPO_3', 'TIPO_4'])
  for (const position of ['ARRIBA', 'ABAJO', 'ARRIBA_Y_ABAJO'])
    test(`troquelado ${type} ${position}`, () => {
      const { object, success } = createLockerInstance({
        config: { punchingType: type, punchingPosition: position },
      });
      assert.equal(success, true);
      const punching = object.getObjectByName('PUNCHING');
      assert.ok(punching.children.length > 0);
      assert.equal(punching.userData.punchingType, type);
      assert.equal(punching.userData.punchingPosition, position);
      assert.ok(
        punching.userData.approximationFlags.includes('LOCKER_NATIVE_DIMENSION_APPROXIMATED')
      );
      disposeLockerGeometry(object);
    });

test('manijas con códigos diferenciados por material y posiciones dependientes de altura/naves', () => {
  for (const handleType of ['EMBEBIDA', 'BOTON', 'SCHWINN', 'INCRUSTAR', 'SIN_MANIJA'])
    for (const material of ['METALICA', 'FORMICA', 'MELAMINA']) {
      const result = createLockerInstance({
        config: {
          handleType,
          material,
          ...(material === 'METALICA' ? {} : { punchingType: 'NO_APLICA' }),
        },
      });
      assert.equal(result.success, true);
      assert.equal(
        result.object.userData.bom.some((r) => r.role === 'HANDLE'),
        handleType !== 'SIN_MANIJA'
      );
      disposeLockerGeometry(result.object);
    }
  assert.equal(
    resolveLockerProduct({ handleType: 'BOTON' }).accessories.handle.code,
    '22000128635'
  );
  assert.equal(
    resolveLockerProduct({ material: 'FORMICA', punchingType: 'NO_APLICA', handleType: 'BOTON' })
      .accessories.handle.code,
    '22000128769'
  );
  const p = {
    bayIndex: 0,
...
test('guardar/cargar conserva identidad, configuración, acabados y transformaciones en proyecto mixto', async () => {
  const original = createLockerInstance({
    config: {
      bayCount: 3,
      bayFinishes: ['#ff0000', '#ffffff', '#0000ff'],
      visualState: { doorDisplay: 'CLOSED' },
    },
    transform: { position: [1, 0, 2], rotation: [0, 0.7, 0], scale: [1.2, 1, 1] },
  }).object;
  const other = new THREE.Group();
  other.userData = { kind: 'PART', codigoPT: 'OTHER' };
  const serialized = serializeProjectEntities([
    { obj: original },
    { obj: original },
    { obj: other },
  ]);
  ...
  const restored = await loadPersistedEntity(entity, {
    createLocker: (e) =>
      createLockerInstance({ config: e.config, instanceId: e.instanceId, transform: e.transform })
        .object,
  });
  assert.deepEqual(restored.userData.config, original.userData.config);
  assert.equal(restored.userData.instanceId, original.userData.instanceId);
  assert.deepEqual(restored.position.toArray(), original.position.toArray());
  assert.deepEqual(restored.quaternion.toArray(), original.quaternion.toArray());
  assert.deepEqual(restored.scale.toArray(), original.scale.toArray());
  assert.deepEqual(restored.userData.bom, original.userData.bom);
  const parent = new THREE.Group(),
    parts = [],
    pickables = [];
  registerLockerInstance({
    instance: { object: restored },
    parent,
    partsRegistry: parts,
    pickables,
  });
  ...
  disposeLockerGeometry(original);
  disposeLockerGeometry(restored);
});

test('clipboard usa configuración serializable y constructor LOCKER, con identidad nueva', () => {
  const { object } = createLockerInstance({
    config: { bayCount: 2, bayFinishes: ['#123456', '#abcdef'] },
  });
  const entity = serializeLockerEntity(object);
  const instruction = createObjectFromClipboardItem({
    kind: entity.kind,
    transform: entity.transform,
    configuration: { config: entity.config },
    relationships: { oldId: entity.instanceId },
  });
  ...
  assert.notEqual(copied.userData.instanceId, object.userData.instanceId);
  assert.deepEqual(copied.userData.config, object.userData.config);
  disposeLockerGeometry(object);
  disposeLockerGeometry(copied);
});

test('historial global: una operación completa de configuración, undo y redo conservan raíz', async () => {
  const { object } = createLockerInstance();
  const before = structuredClone(object.userData.config);
  const history = createHistoryManager({
    replayAction: (action, direction) => {
      const r = rebuildLockerInstance({
        object,
        patch: direction === 'undo' ? action.before : action.after,
      });
  ...
  history.pushAction({
    ...
    before,
    after: structuredClone(object.userData.config),
  });
  await history.undo();
  assert.equal(object.userData.heightMm, 1800);
  assert.equal(bays(object).length, 1);
  await history.redo();
  assert.equal(object.userData.heightMm, 2200);
  assert.equal(bays(object).length, 4);
  disposeLockerGeometry(object);
});
